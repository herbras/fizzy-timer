...
export const fizzyKeys = {
	...
	columns: (accountSlug: string, boardId: string) =>
		["fizzy", "columns", accountSlug, boardId] as const,
	cards: (
		accountSlug: string,
		boardId?: string | null,
		userId?: string | null,
...
export function useBoards(accountSlug: string | null) {
	...
	return useQuery({
		queryKey: accountSlug
			? fizzyKeys.boards(accountSlug)
			: ["fizzy", "boards", "none"],
		queryFn: async () => {
...
export function useColumns(accountSlug: string | null, boardId: string | null) {
	...
	return useQuery({
		queryKey:
			accountSlug && boardId
				? fizzyKeys.columns(accountSlug, boardId)
				: ["fizzy", "columns", "none"],
		queryFn: async () => {
...
export function toBoard(apiBoard: ApiBoard): Board {
	...
	const colorIndex = Number.parseInt(apiBoard.id.slice(-4), 16) % colors.length;
