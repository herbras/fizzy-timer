...
import {
	useDeleteSession as useDeleteSessionMutation,
	useSessions as useSessionsQuery,
...
export function useTodaySessions() {
	const { data: queryData, isLoading: loading, refetch } = useSessionsQuery();
	const sessions = (queryData as Session[]) || [];
...
export function useSessionsByDate() {
	...
	const grouped = useMemo(() => {
		// First, group by date
		const byDate: Record<string, Session[]> = sessions.reduce(
			(acc, session) => {
		...
		for (const [dateKey, dateSessions] of Object.entries(byDate)) {
			const cardMap = new Map<
				string,
				{ session: Session; count: number; ids: string[] }
			>();
			...
			for (const session of dateSessions) {
				...
				if (cardMap.has(cardKey)) {
					...
					existing.session.startTime = Math.min(
						existing.session.startTime,
						session.startTime,
					);
					if (session.endTime) {
						existing.session.endTime = Math.max(
							existing.session.endTime || 0,
							session.endTime,
						);
					}
					existing.count += 1;
			...
			merged[dateKey] = Array.from(cardMap.values()).map(
				({ session, count, ids }) => ({
					...session,
					sessionCount: count,
		...
		async (id: string) => {
			...
			for (const dateSessions of Object.values(grouped)) {
				...
				if (session?.mergedIds) {
					...
					await Promise.all(
						session.mergedIds.map((sid) =>
							deleteSessionMutation.mutateAsync(sid),
						),
					);
				} else {
