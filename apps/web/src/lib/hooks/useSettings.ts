...
import { settingsOps, SETTINGS_KEYS } from '../services/settings';
import { accountOps } from '../db/indexed-db';
import { accountKeys } from '../queries/useAccounts';
import { useSwitchAccount, useAddAccount, useRemoveAccount } from '../queries/useAccounts';
...
export function useSettings(): SettingsState & SettingsActions {
  ...
  const { data: deviceId = null } = useQuery({
    ...
    staleTime: Infinity, // Device ID never changes
    gcTime: Infinity,
  });
   ...
   */
  const addAccount = useCallback(async (token: string) => {
    try {
