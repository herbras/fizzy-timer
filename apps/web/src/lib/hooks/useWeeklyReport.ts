 ...
 */
function useFizzyCards(accountSlug: string | null, token: string | null, userId: string | null) {
  return useQuery({
...
export function useWeeklyReport() {
  ...
  const weekStart = useMemo(() => getThursdayStart(selectedWeek), [selectedWeek]);
  const weekEnd = useMemo(() => getThursdayEnd(selectedWeek), [selectedWeek]);

  // Fetch all CLOSED cards from Fizzy API for the current user
  const {
    data: allCards = [],
    isLoading: loadingCards,
    error: cardsError,
    refetch: refetchCards,
  } = useFizzyCards(activeAccount?.slug ?? null, activeAccount?.token ?? null, activeAccount?.userId ?? null);
  ...
  const reportData = useMemo((): WeeklyReportData => {
    ...
    const mostActiveDayName = mostActiveDay.cardCount > 0 ? mostActiveDay.displayDate : null;

    // Most active board
    const boardCounts = new Map<string, number>();
    for (const card of weekCards) {
      boardCounts.set(card.boardName, (boardCounts.get(card.boardName) || 0) + 1);
    }
    const mostActiveBoardEntry = Array.from(boardCounts.entries()).sort((a, b) => b[1] - a[1])[0];
    const mostActiveBoard = mostActiveBoardEntry ? mostActiveBoardEntry[0] : null;
 ...
 */
export function generateWeeklyPrompt(data: WeeklyReportData, userName?: string): string {
  const lines: string[] = [];
  ...
  if (data.mostActiveBoard) {
    lines.push(`    <most_active_board>${data.mostActiveBoard}</most_active_board>`);
  }
  lines.push(`  </summary>`);
  ...
  if (data.dailyData.some((d) => d.cardCount > 0)) {
    ...
    for (const day of data.dailyData) {
      if (day.cardCount > 0) {
        lines.push(
          `    <day date="${day.date}" cards="${day.cardCount}"/>`
        );
      }
    }
    lines.push(`  </daily_breakdown>`);
  ...
  if (data.boards.length > 0) {
    ...
    for (const boardName of data.boards) {
      ...
      lines.push(`    <board name="${boardName}" cards="${boardCards.length}">`);
      for (const card of boardCards) {
        const assignees = card.assignees.length > 0 ? ` assignees="${card.assignees.join(', ')}"` : '';
        const tags = card.tags.length > 0 ? ` tags="${card.tags.join(', ')}"` : '';
        lines.push(`      <card number="${card.cardNumber}" title="${card.cardTitle}" column="${card.column}"${assignees}${tags}/>`);
      }
      lines.push(`    </board>`);
    }
    lines.push(`  </boards>`);
 ...
 */
export function generateWhatsAppText(data: WeeklyReportData, userName?: string): string {
  const lines: string[] = [];
 ...
 */
export async function export4WeeksToExcel(allCards: CardStats[]): Promise<void> {
  ...
  try {
    ...
    const weeksData: { label: string; start: Date; end: Date; cards: CardStats[] }[] = [];
    ...
    const summaryData = [
      ...
      [''],
      ['Week', 'Total Cards', 'Total Boards', 'Avg Daily Cards', 'Most Active Board'],
    ];
    ...
    weeksData.reverse().forEach((week) => {
      ...
      const mostActiveBoard = Array.from(boardCounts.entries())
        .sort((a, b) => b[1] - a[1])[0]?.[0] || '-';

      summaryData.push([week.label, totalCards, totalBoards, avgDaily, mostActiveBoard]);
    });
    ...
    weeksData.reverse().forEach((week, index) => {
      ...
      const weekData = [
        [`Week ${index + 1}: ${week.label}`],
        [''],
        ['No', 'Card Number', 'Title', 'Board', 'Column', 'Completed At', 'Assignees', 'Tags'],
      ];
