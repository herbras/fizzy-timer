...
import type { Card, Board } from '../db/schema';
...
function getSavedTimerState(): { data: TimerData | null; startTime: number | null; elapsed: number; previousElapsed: number; mode: TimerMode; duration?: number } {
  try {
    ...
    if (saved) {
      ...
      if (parsed.startTime && parsed.state === 'running') {
        const currentSessionElapsed = Math.floor((Date.now() - parsed.startTime) / 1000);
        return {
          data: parsed.data,
    ...
    console.warn('[useTimer] Failed to read timer state from localStorage:', error);
  }
  return { data: null, startTime: null, elapsed: 0, previousElapsed: 0, mode: 'stopwatch' };
}
...
export function useTimer(): UseTimerReturn {
  ...
  useEffect(() => {
    ...
    if (saved.data) {
      ...
      if (saved.startTime) {
        ...
        const currentElapsed = Math.floor((Date.now() - saved.startTime) / 1000);
        setState('running');
  ...
  useEffect(() => {
    ...
    const saveTimer = () => {
      saveTimerState(state, data, startTimeRef.current, currentSessionElapsed, previousElapsedRef.current, mode, durationRef.current);
    };
  ...
  const start = useCallback((timerData: TimerData, durationSeconds?: number) => {
