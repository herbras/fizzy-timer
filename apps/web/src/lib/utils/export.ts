 ...
 */
function calculateSummary(sessions: Session[], options: ExportOptions): ExportSummary {
  const now = new Date();
 ...
 */
export function exportToExcel(sessions: Session[], options: ExportOptions = {}): void {
  ...
  const summaryData = [
    ...
    ['', '', '', ''],
    ['Total Hours (Decimal)', formatDurationDecimal(
  ...
  sessions.forEach((session) => {
    const existing = boardMap.get(session.boardName) || { duration: 0, sessions: 0 };
  ...
  const cardMap = new Map<
    string,
    { board: string; title: string; duration: number; sessions: number; number: number }
  >();
  sessions.forEach((session) => {
  ...
  const cardData = [
    ['SUMMARY BY CARD', '', '', '', '', '', ''],
    ['', '', '', '', '', '', ''],
    ['Card Number', 'Card Title', 'Board', 'Total Sesi', 'Total Jam (Decimal)', 'Total Jam (Format)', ''],
    ['-', '-', '-', '-', '-', '-', '-'],
  ...
  // Generate filename
  const monthStr = summary.month ? summary.month.toLowerCase().replace(' ', '-') : 'all';
 ...
 */
export function exportToCSV(sessions: Session[], options: ExportOptions = {}): void {
  const summary = calculateSummary(sessions, options);
  ...
  csvRows.push('"Board","Total Sesi","Total Jam (Decimal)","Total Jam (Format)"');
 ...
 */
export function getSessionDateRange(sessions: Session[]): { start: Date; end: Date } | null {
 ...
 */
export function filterSessionsByMonth(sessions: Session[], month: Date): Session[] {
