...
import type { Session, UserSettings, StoredAccount } from './schema';
...
export class FizzyDB extends Dexie {
  ...
  constructor() {
    ...
    this.version(2).stores({
      ...
      console.log('[DB Migration] Upgrading to v2 - adding multi-account support');
      ...
      if (token && slug && userId) {
        ...
        console.log('[DB Migration] Migrated', sessions.length, 'sessions to account', accountId);
      } else {
        console.log('[DB Migration] No existing account found, skipping session migration');
      }
    });
  }
}
