...
import type { FizzyAccount, FizzyBoard, FizzyCard, Column } from "../db/schema";
...
const CardSchema = Schema.Struct({
  ...
  assignees: Schema.optionalWith(Schema.NullOr(Schema.Array(CardAssigneeSchema)), { exact: true }),
...
export type IdentityResponse = Schema.Schema.Type<typeof IdentityResponseSchema>;
...
const unwrapBoards = (response: BoardsResponse): ApiBoard[] => {
  ...
  if ('boards' in response && Array.isArray(response.boards)) return response.boards;
...
const unwrapColumns = (response: ColumnsResponse): ApiColumn[] => {
  if (Array.isArray(response)) return response;
  if ('columns' in response && Array.isArray(response.columns)) return response.columns;
  if ('data' in response && Array.isArray(response.data)) return response.data;
  return [];
};
...
const unwrapCards = (response: CardsResponse): ApiCard[] => {
  if (Array.isArray(response)) return response;
  if ('cards' in response && Array.isArray(response.cards)) return response.cards;
  if ('data' in response && Array.isArray(response.data)) return response.data;
  return [];
};
...
export class FizzyConfigError extends Data.TaggedError("FizzyConfigError")<{ message: string }> {}
export class FizzyAuthError extends Data.TaggedError("FizzyAuthError")<{ message: string; status: number }> {}
export class FizzyNetworkError extends Data.TaggedError("FizzyNetworkError")<{ message: string; cause?: unknown }> {}
export class FizzyNotFoundError extends Data.TaggedError("FizzyNotFoundError")<{ message: string; resource: string }> {}
export class FizzyValidationError extends Data.TaggedError("FizzyValidationError")<{ message: string; errors?: Record<string, string[]> }> {}
export class FizzyParseError extends Data.TaggedError("FizzyParseError")<{ message: string; cause?: unknown }> {}
...
export class FizzyConfigService extends Context.Tag("FizzyConfigService")<FizzyConfigService, FizzyConfig>() {}
...
const handleResponse = <A>(
  ...
  return Effect.gen(function* () {
    if (response.status === 401) {
      yield* Effect.fail(new FizzyAuthError({ message: "Invalid API token", status: 401 }));
    }
    if (response.status === 403) {
      yield* Effect.fail(new FizzyAuthError({ message: "Forbidden", status: 403 }));
    }
    if (response.status === 404) {
      yield* Effect.fail(new FizzyNotFoundError({ message: "Not found", resource: response.url }));
    }
    if (!response.ok) {
      yield* Effect.fail(new FizzyNetworkError({ message: `HTTP error: ${response.status}` }));
    }
    ...
    const json = yield* Effect.tryPromise({
      try: () => response.json(),
      catch: (e) => new FizzyParseError({ message: "Parse JSON failed", cause: e }),
    });
...
const fetchWithAuth = (
  ...
  return Effect.gen(function* () {
    ...
    return yield* Effect.tryPromise({
      try: () => fetch(proxyUrl, { ...options, headers: buildHeaders(config.token) }),
      catch: (e) => new FizzyNetworkError({ message: "Network error", cause: e }),
    });
  });
};
...
export const createFizzyClient = (token: string) => {
  ...
  return {
    ...
    getIdentity: () =>
      run(
        Effect.gen(function* () {
          const response = yield* fetchWithAuth(`${FIZZY_BASE_URL}/my/identity`);
    ...
    getBoards: (accountSlug: string) =>
      run(
        Effect.gen(function* () {
          const response = yield* fetchWithAuth(`${FIZZY_BASE_URL}/${accountSlug}/boards`);
    ...
    getColumns: (accountSlug: string, boardId: string) =>
      run(
        Effect.gen(function* () {
          const response = yield* fetchWithAuth(`${FIZZY_BASE_URL}/${accountSlug}/boards/${boardId}/columns`);
     ...
     */
    getCards: (accountSlug: string, options?: { boardId?: string; assigneeId?: string }) =>
      run(
        Effect.gen(function* () {
          ...
          const response = yield* fetchWithAuth(`${FIZZY_BASE_URL}/${accountSlug}/cards${query}`);
    ...
    getCard: (accountSlug: string, cardNumber: number) =>
      run(
        Effect.gen(function* () {
          const response = yield* fetchWithAuth(`${FIZZY_BASE_URL}/${accountSlug}/cards/${cardNumber}`);
