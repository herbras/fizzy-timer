import type { ConvexQueryClient } from "@convex-dev/react-query";
import type { QueryClient } from "@tanstack/react-query";
import { QueryClientProvider } from "@tanstack/react-query";
import {
	createRootRouteWithContext,
	HeadContent,
	Outlet,
	Scripts,
	useNavigate,
} from "@tanstack/react-router";
import { TanStackRouterDevtools } from "@tanstack/react-router-devtools";
import { ConvexProvider } from "convex/react";
import { useEffect, useRef } from "react";

import { Toaster } from "@/components/ui/sonner";
import { settingsOps } from "@/lib/db/indexed-db";
import "@/lib/pwa";

import appCss from "../index.css?url";

export interface RouterAppContext {
	queryClient: QueryClient;
	convexQueryClient: ConvexQueryClient;
}

export const Route = createRootRouteWithContext<RouterAppContext>()({
	head: () => ({
		meta: [
			{
				charSet: "utf-8",
			},
			{
				name: "viewport",
				content:
					"width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no, viewport-fit=cover",
			},
			{
				name: "theme-color",
				content: "#f59e0b",
			},
			{
				title: "Tizzy",
			},
			{
				name: "description",
				content:
					"Track time on your tasks with Tizzy, a companion that matches your mood.",
			},
			// iOS PWA meta tags
			{
				name: "apple-mobile-web-app-capable",
				content: "yes",
			},
			{
				name: "apple-mobile-web-app-status-bar-style",
				content: "black-translucent",
			},
			{
				name: "apple-mobile-web-app-title",
				content: "Tizzy",
			},
			// Microsoft tiles
			{
				name: "msapplication-TileColor",
				content: "#050505",
			},
			{
				name: "msapplication-config",
				content: "/browserconfig.xml",
			},
			// Prevent automatic text size adjustment on iOS
			{
				name: "format-detection",
				content: "telephone=no",
			},
		],
		links: [
			{
				rel: "stylesheet",
				href: appCss,
			},
			{
				rel: "manifest",
				href: "/manifest.webmanifest",
			},
			// Apple touch icons for iOS (iPhone 14, iPad, etc.)
			{
				rel: "apple-touch-icon",
				sizes: "180x180",
				href: "/ios/180.png",
			},
			{
				rel: "apple-touch-icon",
				sizes: "152x152",
				href: "/ios/152.png",
			},
			{
				rel: "apple-touch-icon",
				sizes: "144x144",
				href: "/ios/144.png",
			},
			{
				rel: "apple-touch-icon",
				sizes: "128x128",
				href: "/ios/128.png",
			},
			{
				rel: "apple-touch-icon",
				sizes: "96x96",
				href: "/android/android-launchericon-96-96.png",
			},
			// Favicon
			{
				rel: "icon",
				type: "image/png",
				sizes: "32x32",
				href: "/ios/32.png",
			},
			{
				rel: "icon",
				type: "image/png",
				sizes: "16x16",
				href: "/ios/16.png",
			},
			{
				rel: "shortcut icon",
				href: "/ios/180.png",
			},
		],
	}),

	component: RootDocument,
});

function RootDocument() {
	const { convexQueryClient, queryClient } = Route.useRouteContext();
	const navigate = useNavigate();
	const hasCheckedSetup = useRef(false);

	// Check if setup is complete, redirect to /setup if not
	// Also redirect to /home if logged in user visits root on initial load
	useEffect(() => {
		// Only check once to prevent race conditions and infinite loops
		if (hasCheckedSetup.current) return;
		hasCheckedSetup.current = true;

		const checkSetup = async () => {
			const token = await settingsOps.getFizzyToken();
			const slug = await settingsOps.getAccountSlug();
			const pathname = window.location.pathname;

			// Allow access to /setup without auth
			if (pathname === "/setup") {
				return;
			}

			// Special handling for root route
			if (pathname === "/") {
				// If logged in, redirect to home (only on initial load)
				if (token && slug) {
					navigate({ to: "/home", replace: true });
				}
				// If not logged in, show landing page
				return;
			}

			// For other routes, redirect to setup if not configured
			if (!token || !slug) {
				navigate({ to: "/setup" });
			}
		};

		checkSetup();
	}, [navigate]);

	return (
		<QueryClientProvider client={queryClient}>
			<ConvexProvider client={convexQueryClient.convexClient}>
				<html lang="en">
					<head>
						<HeadContent />
					</head>
					<body className="bg-[#050505]">
						<Outlet />
						<Toaster richColors />
						<TanStackRouterDevtools position="bottom-left" />
						<Scripts />
					</body>
				</html>
			</ConvexProvider>
		</QueryClientProvider>
	);
}
